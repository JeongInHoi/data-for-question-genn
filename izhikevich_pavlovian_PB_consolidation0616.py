...
from common_PB_consolidation0616 import (izhikevich_dopamine_model, izhikevich_stdp_model, 
...
parser.add_argument("--load-file-name", type = str, default = "D-0.001(33)", help = "this simulation will take the initial weight from the load file and proceed with it.")
args = parser.parse_args(args = [])
base_record_directory = 'init_weight_from_'+args.load_file_name+'___D'+str(args.background_D)
record_directory = base_record_directory
counter = 1
while os.path.exists(os.path.join('./simulation_by_different_dopamine',record_directory)):
	record_directory = f"{base_record_directory}({counter})"
	counter +=1

record_directory = os.path.join('./simulation_by_different_dopamine', record_directory)
...
while model.t < params["duration_ms"]:
    ...
    if params["use_genn_recording"]:
	    ...
	    #i_curr_pop.pull_var_from_device()
            ...
            np.save(record_directory+'/'+"ind_e_e_" + record_number+".npy", ind_e_e)
            e_i_pop.pull_connectivity_from_device()
            ind_e_i = np.vstack((e_i_pop.get_sparse_pre_inds(), e_i_pop.get_sparse_post_inds()+no_exc))
            np.save(record_directory+'/'+"ind_e_i_" + record_number +".npy", ind_e_i)
            ...
            np.save(record_directory+'/'+"ind_i_i_" + record_number+".npy", ind_i_i)
            i_e_pop.pull_connectivity_from_device()
            ind_i_e = np.vstack((i_e_pop.get_sparse_pre_inds()+no_exc, i_e_pop.get_sparse_post_inds()))
            np.save(record_directory+'/'+"ind_i_e_" + record_number+".npy", ind_i_e)

            
            ind_tot = np.concatenate((ind_e_e, ind_e_i, ind_i_e, ind_i_i), axis = 1)
            np.save(record_directory+'/'+"ind_tot_"+ record_number+".npy", ind_tot)
